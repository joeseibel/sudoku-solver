...
class FinnedSwordfishTest extends FunSuite:
  test("test 1") {
    val board =
      """
        |6{379}5418{379}{37}2
        ...
        |""".stripMargin.replace("\n", "")
    val expected = Seq(RemoveCandidates(6, 3, 3))
  ...
  test("test 2") {
    val board =
      """
        ...
        |""".stripMargin.replace("\n", "")
    val expected = Seq(RemoveCandidates(7, 0, 3))
    assertLogicalSolution(expected, board, finnedSwordfish)
  }
  ...
  test("test 4") {
    val board =
      """
        ...
        |68{359}{257}{2357}{579}{237}41
        |""".stripMargin.replace("\n", "")
    val expected = Seq(RemoveCandidates(7, 5, 7))
    assertLogicalSolution(expected, board, finnedSwordfish)
  }
  ...
  test("test 5") {
    val board =
      """
        ...
        |""".stripMargin.replace("\n", "")
    val expected = Seq(RemoveCandidates(5, 4, 6))
    assertLogicalSolution(expected, board, finnedSwordfish)
  }
  ...
  test("test 6") {
    val board =
      """
        ...
        |1{38}5{378}2964{78}
        |""".stripMargin.replace("\n", "")
    val expected = Seq(RemoveCandidates(7, 0, 6))
    assertLogicalSolution(expected, board, finnedSwordfish)
  }