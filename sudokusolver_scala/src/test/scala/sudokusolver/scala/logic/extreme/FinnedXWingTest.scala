...
class FinnedXWingTest extends FunSuite:
  test("test 1") {
    val board =
      """
        ...
        |""".stripMargin.replace("\n", "")
    val expected = Seq(RemoveCandidates(7, 8, 7))
  ...
  test("test 2") {
    val board =
      """
        ...
        |""".stripMargin.replace("\n", "")
    val expected = Seq(RemoveCandidates(5, 0, 3, 5))
    assertLogicalSolution(expected, board, finnedXWing)
  }
  ...
  test("test 3") {
    val board =
      """
        ...
        |{124}{146}{126}859{24}73
        |""".stripMargin.replace("\n", "")
    val expected = Seq(RemoveCandidates(5, 0, 5))
    assertLogicalSolution(expected, board, finnedXWing)
  }
