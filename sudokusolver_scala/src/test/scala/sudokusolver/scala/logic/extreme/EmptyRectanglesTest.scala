...
class EmptyRectanglesTest extends FunSuite:
  test("test 1") {
    val board =
      """
        ...
        |{36}{16}{136}279854
        |""".stripMargin.replace("\n", "")
    val expected = Seq(RemoveCandidates(3, 8, 6))
  ...
  test("test 3") {
    val board =
      """
        ...
        |""".stripMargin.replace("\n", "")
    val expected = Seq(RemoveCandidates(8, 4, 3))
    assertLogicalSolution(expected, board, emptyRectangles)
  }
  ...
  test("test 5") {
    val board =
      """
        ...
        |""".stripMargin.replace("\n", "")
    val expected = Seq(RemoveCandidates(6, 7, 3))
    assertLogicalSolution(expected, board, emptyRectangles)
  }