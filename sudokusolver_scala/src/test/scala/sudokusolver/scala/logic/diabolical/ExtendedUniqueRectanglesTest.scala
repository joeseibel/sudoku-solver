...
class ExtendedUniqueRectanglesTest extends FunSuite:
  test("test 1") {
    val board =
      """
        |9{16}7{18}243{68}5
        ...
        |428673591
        |""".stripMargin.replace("\n", "")
    val expected = Seq(RemoveCandidates(2, 0, 1, 5))
  ...
  test("test 2") {
    val board =
      """
        ...
        |397{56}4{56}128
        |""".stripMargin.replace("\n", "")
    val expected = Seq(RemoveCandidates(4, 7, 4, 7))
    assertLogicalSolution(expected, board, extendedUniqueRectangles)
  }
  ...
  test("test 3") {
    val board =
      """
        ...
        |""".stripMargin.replace("\n", "")
    val expected = Seq(RemoveCandidates(3, 1, 3, 8))
    assertLogicalSolution(expected, board, extendedUniqueRectangles)
  }