...
class UniqueRectanglesTest extends FunSuite:
  test("type 1 test 1") {
    val board =
      """
        ...
        |{146}{14}82597{36}{346}
        |""".stripMargin.replace("\n", "")
    val expected = Seq(RemoveCandidates(3, 0, 2, 9))
  ...
  test("type 1 test 2") {
    val board =
      """
        ...
        |165748923
        |""".stripMargin.replace("\n", "")
    val expected = Seq(RemoveCandidates(0, 6, 2, 5))
    assertLogicalSolution(expected, board, uniqueRectanglesType1)
  }
  ...
  test("type 1 test 3") {
    val board =
      """
        ...
        |""".stripMargin.replace("\n", "")
    val expected = Seq(RemoveCandidates(3, 0, 7, 8))
    assertLogicalSolution(expected, board, uniqueRectanglesType1)
  }
  ...
  test("type 1 test 4") {
    val board =
      """
        ...
        |2347{68}15{68}9
        |""".stripMargin.replace("\n", "")
    val expected = Seq(RemoveCandidates(6, 4, 6, 8))
    assertLogicalSolution(expected, board, uniqueRectanglesType1)
  }
  ...
  test("type 2B test") {
    val board =
      """
        ...
        |""".stripMargin.replace("\n", "")
    val expected = Seq(RemoveCandidates(1, 6, 8))
    assertLogicalSolution(expected, board, uniqueRectanglesType2)
  }
  ...
  test("type 3B with triple pseudo cells test 1") {
    val board =
      """
        |7529{36}8{16}4{136}
        ...
        |""".stripMargin.replace("\n", "")
    val expected = Seq(RemoveCandidates(7, 0, 6, 9))
    assertLogicalSolution(expected, board, uniqueRectanglesType3BWithTriplePseudoCells)
  }

  test("type 3B with triple pseudo cells test 2") {
    val board =
      """
        |654728{19}{139}{39}
        |321964{58}7{58}
