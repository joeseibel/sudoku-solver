...
class XYZWingTest extends FunSuite:
  test("test 1") {
    val board =
      """
        |{38}92{46}{48}175{346}
        ...
        |""".stripMargin.replace("\n", "")
    val expected = Seq(RemoveCandidates(5, 6, 1))
  ...
  test("test 2") {
    val board =
      """
        ...
        |""".stripMargin.replace("\n", "")
    val expected = Seq(RemoveCandidates(4, 8, 6))
    assertLogicalSolution(expected, board, xyzWing)
  }
  ...
  test("test 3") {
    val board =
      """
        ...
        |""".stripMargin.replace("\n", "")
    val expected = Seq(RemoveCandidates(8, 4, 5))
    assertLogicalSolution(expected, board, xyzWing)
  }
