...
func bug(board: Board<Cell>) -> BoardModification? {
    ...
    if let cell = cells.first, cells.count == 1 && cell.candidates.count == 3 {
        ...
        let candidates = cell.candidates.filter { candidate in row.count { $0.candidates.contains(candidate) } == 3 }
        precondition(candidates.count == 1)
        return BoardModification(cell: cell, value: candidates.first!)
    } else {
        return nil
    }
}
